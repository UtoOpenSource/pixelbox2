...
static class fps_info_man : public UIInstance {
 public:
...
UIInstance* fps_overlay = &fps_man;
};	// namespace ui
...
#include "clock.hpp"
void pb::screen::ui::fps_info_man::collect() { story.add(pb::__clocksource.time()); }
...
static void draw_summary(pb::screen::frametime_summary& s, bool alt = false, double period = 1) {
	...
	if (alt)
		ImGui::Text("%03i FPS (%03ims)", s.real_fps, int((1.0 / double(s.real_fps)) * 1000));
	else
		ImGui::Text("%03i FPS", s.real_fps);

#define COLV(v)                                                                  \
	ImGui::TableNextColumn();                                                      \
	if (alt)                                                                       \
		ImGui::Text("%03ims (%03i)", int(s.v * 1000), s.v > 0 ? int(1.0 / s.v) : 0); \
	else                                                                           \
		ImGui::Text("%03ims", int(s.v * 1000));
	COLV(min);
	COLV(avg);
	COLV(max);
#undef COLV
...
void pb::screen::ui::fps_info_man::draw() {
